...
void runExperiment(TspBoard &gb, std::string type, bool &checkLog, std::vector<int> &result, std::vector<int> &expansion, std::vector<std::string> &version,
    ...
    if (type == "Astar") {
        ...
        res = fs.AStarSearch(gb, heuristic, num_expansion, num_iteration, dist_matrix);
        std::cout << "AStarSearch\n"
                  << res << '\n';
        std::cout << "AStarSearch with num_expansion is " << num_expansion << '\n';
        std::cout << type << " with num_iteration is " << num_iteration << '\n';

    } else if (type == "FocalSearch") {
        // BasicFocalSearch<TspBoard> fs;
        // res = fs.FocalSearch(gb, open_value, focal_value, heuristic, num_expansion, num_iteration, dist_matrix);

        ProbabilityFocalSearch<TspBoard> fs;
        res = fs.ProbabilitySearch(gb, open_value, focal_value, heuristic, num_expansion, num_iteration, dist_matrix, 1.1, 1.0, 1.0);
        ...
        ProbabilityFocalSearch<TspBoard> fs;
        res = fs.ProbabilitySearch(gb, open_value, focal_value, heuristic, num_expansion, num_iteration, dist_matrix, 1.1, 1.0, pickRate);
...
void TspDemo(int size) {
    ...
    std::string fileName = "tsp_20_100_1";
    ...
    for (int i = 1; i <= test; ++i) {
        ...
        runExperiment(tsp, "ProbabilityFocalSearch 100/0", checkLog, result, expansion, version, open_funct, focal_funct, MST, dist_matrix, 1.0);
        runExperiment(tsp, "FocalSearch", checkLog, result, expansion, version, open_funct, focal_funct, MST, dist_matrix);
        //runExperiment(tsp, "ProbabilityFocalSearch 60/40", checkLog, result, expansion, version, open_funct, focal_funct, MST, dist_matrix, 1.0);
        ...
        // tmux
        // ./fs_prototype &
        // noi chung nhu the, ko lo bi tat vscode
        // compile lai code de chay cai cau hinh khac song song neu muon
        if (checkLog) {
            logFile << "state,";
            for (auto v : version) {
                logFile << v << " result,";
        ...
        // chi chay cai configuration nao tot nhat thoi
        // ko can chay qua nhieu cai configuration cho PFS
        logFile << tsp << ",";
...
int main() {
    ...
    // benchmark::genInput(25, 100);
    benchmark::TspDemo(25);
    return 0;
}
