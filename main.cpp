...
namespace benchmark
{
    ...
    void runExperiment(PancakeBoard&gb,std::string type, bool &checkLog, std::vector<int> &result, std::vector<int> &expansion,std::vector<std::string> &version ,
    ...
    )
    {
       ...
       if(type=="Astar"){
                ...
                BasicFocalSearch<PancakeBoard> fs;
                ...
                ProbabilityFocalSearch<PancakeBoard> fs;
    ...
    void NPuzzleDemo(int size)
    {
        ...
        std::string fileName = "pancake_20_20_1";
       ...
       // std::cout << newFolder;
        ...
        for (int i = 1; i <=test; ++i)
        {
            ...
            //First parameter for size of board, second paramater for number of steps moving from initial state
             ...
             PancakeBoard gb=PancakeBoard(game);
             result.clear();
             expansion.clear();


            // runExperiment(gb, "Astar", checkLog, result, expansion, version,open_funct,focal_funct,ManhattanDistance);
             runExperiment(gb, "FocalSearch", checkLog, result, expansion, version, open_funct, focal_funct, simpleHeuristic);
             runExperiment(gb, "ProbabilityFocalSearch 60/40", checkLog, result, expansion, version, open_funct, focal_funct, simpleHeuristic,0.6);
             runExperiment(gb, "ProbabilityFocalSearch 70/30", checkLog, result, expansion, version, open_funct, focal_funct, simpleHeuristic,0.7);
             runExperiment(gb, "ProbabilityFocalSearch potential func 60/40", checkLog, result, expansion, version, open_funct, focal_potential, simpleHeuristic, 0.6);
             runExperiment(gb, "ProbabilityFocalSearch potential func 70/30", checkLog, result, expansion, version, open_funct, focal_potential, simpleHeuristic, 0.7);
