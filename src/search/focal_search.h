...
class Node {
 public:
  ...
  bool operator<(const Node<G> &oth) const {
    if (f != oth.f) return f < oth.f;
    if (g != oth.g) return g < oth.g;
    if (hFocal != oth.hFocal) return hFocal < oth.hFocal;
...
class BasicAStar {
 ...
 public:
  ...
  inline int AStarSearch(G start, T heuristic, int &num_expansion,
    ...
    open.insert({start.GetHeuristic(heuristic, dist_matrix), (double)0,
                 start.GetHeuristic(heuristic, dist_matrix), (double)0, start});
    visited[start] = 0;
    while (open.size()) {
      ...
      for (G &next_board : GetNeighbour(board)) {
        if (visited.find(next_board) == visited.end() ||
            visited[next_board] >
                g + cost_move(board, next_board, dist_matrix)) {
          visited[next_board] = g + cost_move(board, next_board, dist_matrix);
          double h_new = next_board.GetHeuristic(heuristic, dist_matrix);

          open.insert({g + cost_move(board, next_board, dist_matrix) + h_new,
                       g + cost_move(board, next_board, dist_matrix),
                       static_cast<double>(h_new), 0, next_board});
        }
      }
    }
...
class BasicFocalSearch {
 ...
 public:
  ...
  inline int FocalSearch(G &start, open_funct open_value,
                         focal_funct focal_value, T heuristic,
                         int &num_expansion,
                         std::vector<std::vector<double>> &dist_matrix,
    ...
    const auto nodeValue = [&](double g, G &board) {
      ...
      return Node(open_value(g, h), g, h,
                  focal_value(g, h, board.getDistanceToGo(), C, w), board);
    };
    ...
    while (!open.empty()) {
      ...
      for (G &next_board : GetNeighbour(board)) {
        if (visited.find(next_board) == visited.end() ||
           ...
           */
          Node<G> nextNode = nodeValue(
              g + cost_move(board, next_board, dist_matrix), next_board);
          open.insert(nextNode);
          link_open.emplace(next_board, nextNode);
          if (open_value(g + cost_move(board, next_board, dist_matrix),
                         h_new) <= epsilon * f_min) {
            focal.push(nextNode);
          }
        }
      ...
      if (!open.empty() && f_min < f_head) {
        /*
         * update focal: insert new node from open to focal with epsilon * fmin
         * <= f <= epsilon * f_head
         */
        Node<G> middleNode =
            Node<G>(f_min * epsilon, (double)-1, (double)-1, (double)-1, board);
        for (auto state = open.lower_bound(middleNode); state != open.end();
             ++state) {
          auto board = state->board;
          if (state->f > epsilon * f_head) break;
